...
export default function AbolitionistGazette() {
  const dispatches = articles.filter((a) => a.tag === "Abolition");
  return (
    <div className="relative z-10" style={{ maxWidth: "1140px", margin: "0 auto", padding: "2rem 1.5rem" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: "2rem", paddingBottom: "1rem", borderBottom: "3px double var(--color-walnut)" }}>
        <p style={{ ...S.monoLabel, fontSize: "0.58rem", color: "var(--color-rust)", marginBottom: "0.3rem" }}>Abolitionist Archive</p>
        <h1 style={{ ...S.sectionTitle, fontSize: "clamp(1.8rem, 4vw, 2.6rem)" }}>Abolitionist Dispatches</h1>
      </div>
        ...
        {dispatches.map((a, i) => {
          ...
          return (
            <div key={a.slug} style={{
              ...
              <span style={{ ...S.monoLabel, fontSize: "0.52rem", color: "var(--color-rust)", display: "block", marginBottom: "0.25rem" }}>{a.date}</span>
